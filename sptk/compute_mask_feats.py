...
from libs.utils import get_logger, cmat_abs, abs, EPSILON
...
def compute_mask(speech, noise_or_mixture, mask):
    ...
    if mask == "irm":
        ...
        denominator = np.sqrt(
            abs(speech)**2 + abs(noise_or_mixture)**2)
    elif mask == "crm":
...
def run(args):
    ...
    # modify by kaldiio use dictionary
    speech_reader = kaldiio.load_scp(args.speech_scp)
    denorm_reader = kaldiio.load_scp(args.denorm_scp)
