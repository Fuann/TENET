...
def run(args):
    ...
    if args.phase_ref:
        phase_reader = SpectrogramReader(
            ...
            **stft_kwargs,
            round_power_of_two=args.round_power_of_two)
        #fstring:logger.info(f'Using phase reference from {args.phase_ref}')
        logger.info('Using phase reference from %s', args.phase_ref)
    ...
    with WaveWriter(args.dst_dir, fs=args.sf) as writer:
        for key, specs in spectrogram_reader:
            ...
            if key in mask_reader:
                ...
                #fstring: logger.info(f"Processing utterance {key}...")
                ...
                if mask.shape != specs.shape:
                    raise ValueError(
                        ...
                        "(%s x %s vs " +
                        "%s x %s), need " +
                        "check configures", mask.shape[0], mask.shape[1], specs.shape[0], specs.shape[1])
    ...
    logger.info(
        "Processed %d utterances over %d", num_done, len(spectrogram_reader))
