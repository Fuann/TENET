...
def run(args):
    ...
    with WaveWriter(args.dump_dir, fs=args.sr, normalize=args.normalize) as writer:
        for key, spec in feature_reader:
            ...
            if phase_reader is None:
                ...
                samps = griffin_lim(spec,
                                    ...
                                    norm=0.8,
                                    **stft_kwargs)
                #samps = griffinlim(spec)
            else:
                if key not in phase_reader:
                    raise KeyError("Missing key %s in phase reader", key)
    ...
    logger.info("Processed %d utterance done", len(feature_reader))
